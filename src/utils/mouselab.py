...
from src.utils.distributions import smax, cmax, sample, expectation, Normal, PointMass
...
class MouselabEnv(gym.Env):
    ...
    def __init__(self, tree, init, ground_truth=None, cost=0, term_belief=True, sample_term_reward=False, depth_weight = 0, cost_weight = 1):
        self.tree = tree
        self.mdp_graph = create_base_mdp_graph(self.tree)
        self.depth_dict = nx.shortest_path_length(self.mdp_graph,0)
        self.init = (0, *init[1:])
        ...
        self.selection_cost = -abs(cost)
        self.cost = lambda action : - self.depth_weight * (self.depth_dict[action] ) - self.cost_weight * abs(self.selection_cost) #default is just cost
        self.term_belief = term_belief
        self.sample_term_reward = sample_term_reward
        # Termination of deliberation
    ...
    def _term_reward(self):
        ...
        # Lovis: If this throws a type error make sure ground truth is a numpy array instead of a list
...
def create_base_mdp_graph(tree):
    '''
    Added by VF 3.10.19
    Makes a Networkx graph of the base MDP from the tree for future cost parametrizations
    '''

    #Make the base environment adjacency list from the tree (could probably be more efficient if needed)
    edges = []
    for idx_node, node in enumerate(tree):
        for adjacent_node in node:
            edges.append((idx_node, adjacent_node))
                
    mdp_graph = nx.DiGraph()
    mdp_graph.add_edges_from(edges)
    return mdp_graph
